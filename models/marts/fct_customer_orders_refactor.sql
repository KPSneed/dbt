with paid_orders as (
    ...
    from {{ source('dbt_datasets', 'raw_orders') }}  as orders
    left join (
    ...
    left join {{ source('dbt_datasets', 'raw_customers') }}  c on orders.user_id = c.id 
),
...
customer_orders as (
    ...
    from {{ source('dbt_datasets', 'raw_customers') }}  c 
    left join {{ source('dbt_datasets', 'raw_orders') }} as orders
